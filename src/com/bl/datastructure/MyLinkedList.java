...
public class MyLinkedList<K> {
	...
	/*
	 * public void sorted(INode<K> node) { if (head == null) { add(node); } else {
	 * int index = 0; INode<K> tempNode = head; if (node.compareTo(tempNode) < 0) {
	 * add(node); } else { while (tempNode != null) { index++; if
	 * (tempNode.getNext() != null && node.compareTo(tempNode.getNext()) < 0) {
	 * break; } tempNode = tempNode.getNext(); } insert(node, index); } } }
	 */
